-- 0046: consolidated billing by client rhythm (owner spec 2026-07-28).
--
-- Most clients bill per episode (a deal invoice the moment the episode is
-- approved). A few bill in bulk: monthly, or every N episodes. For those the
-- per-episode chain must FREEZE — the work order does not issue, the deal
-- invoice is never enqueued — until the bookkeeper "redeems" (פדה) the client,
-- which produces ONE consolidated work order + ONE consolidated deal invoice
-- (the existing bundle mechanism, 0044).
--
-- This migration adds only the state the feature needs. The brake itself lives
-- in the app (the two enqueue call sites) and in enqueue.ts — deliberately NOT
-- in on_production_approved, which only ever creates the internal job.
--
...
--    (which is the billing STYLE: per_episode/retainer/package/none); this is
--    the FREQUENCY of consolidation.
do $$ begin
...
-- every_n needs its N; the other cadences must not carry a stray one.
do $$ begin
...
-- 2. two new lifecycle states for a queued document:
--    'accrued'      — frozen by cadence; owed but deliberately not issued yet.
--    'consolidated' — a per-episode row that has been folded into a bundle
--                     document at redemption (kept for audit, never re-issued).
--    NOTE: not referenced in the same statement as the index below, so no
--    "unsafe use of new enum value" — the index predicate names only the two
--    OLD terminal states it must EXCLUDE.
...
-- 3. THE idempotency fix (feasibility Q2/Q6): the one-live-row-per-production
...
--    and consolidated too, and names no new enum value, so it applies cleanly
--    in this same migration. rejected/failed still allow a fresh attempt.
