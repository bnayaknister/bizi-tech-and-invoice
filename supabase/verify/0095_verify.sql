...
select
  ...
  -- 3. TLV לא זז — אולפן של העסק, בשימוש מועט, נשאר כפי שהוא (0095 אינה
  --    נוגעת בו). הספירה **10** נמדדה במצאי שלפני המיגרציה, 22.9.
  --    ⚠️ מספר מדויק ולא `>= 1` בכוונה: `>= 1` היה עובר גם אם 9 מתוך 10
  --    השורות היו נעלמות, כלומר הוא לא היה בודק את מה שהוא קיים בשבילו.
  (select tlv from counts) = 10                                    as tlv_still_there,
