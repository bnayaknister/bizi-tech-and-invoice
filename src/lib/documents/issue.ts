...
// ---------------------------------------------------------------------------
// The parent work order a deal invoice closes.
//
// MOVED to lib/documents/parentRef.ts on 2026-09-17, unchanged. It lived here
// because issue.ts was its only caller; three more arrived the same day (the
// /finance bundle, the registry's "+ חשבון עסקה חדש", and the shared remark
...
export async function issuePendingDocument(
  ...
  const sent: MorningDocumentRequest = {
    ...
    // live 2026-08-02, bundle.ts:203). income and amount are never touched.
    //
    // ⚠️ CORRECTED 2026-09-17. This comment used to add "remarks is left alone
    // too: Morning generates it from the link", and that is false. See
    // types.ts:100-110, which records the live check of 2026-08-04: Morning
    // fills `remarks` for documents raised in its OWN UI (285 in the books) and
    // leaves it null for anything raised through the API. 40321 proved it again
    // on 2026-09-09 — it closed work order 10320 correctly and went out without
    // naming it anywhere on the page, while 40319, built two days earlier by
    // the registry path, printed "חשבון עסקה עבור הזמנה 10321".
    //
    // The remark is also the only route to `parent_doc_numbers` on our side:
    // parseParentLink reads it back off the next daily pull (registry.ts:247).
    // So the missing line cost both halves, and both are restored by sending it.
    ...(parentLink?.linked
      ? parentRefFields(
          buildParentRef({
            childType: row.doc_type,
            parentCode: MORNING_DOC_CODE.order,
            parents: [
              {
                morning_doc_id: parentLink.morning_doc_id,
                morning_doc_number: parentLink.work_order_number,
              },
            ],
          })
        )
