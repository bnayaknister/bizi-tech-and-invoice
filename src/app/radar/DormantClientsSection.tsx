...
// gets its own small client component (owner spec 2026-07-21). Server data in,
  ...
  return (
    <section>
      <h2 className="text-xs font-bold mb-2 flex items-center gap-1.5" style={{ color: "var(--cyan)" }}>
        ...
        לקוחות רדומים (90+ יום ללא הקלטה)
      </h2>
      <div
      ...
      >
        {clients.map((c) => (
          ...
          >
            ...
            <div className="text-[11px] text-[var(--dim)] text-center shrink-0">
              <div>{c.lastRecordDate ?? "מעולם לא הקליט"}</div>
              <div className="font-mono">{c.daysSince != null ? `לפני ${c.daysSince} ימים` : "—"}</div>
            </div>
