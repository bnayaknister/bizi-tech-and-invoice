...
// severity labels carry no emoji (DESIGN.md §12) — a colored dot does the work
    ...
    <div className="min-h-screen">
      ...
      <main className="max-w-4xl mx-auto p-6">
        <h1 className="text-lg font-bold mb-1 flex items-center gap-2.5">
          <IconTile icon="radar" accent="rose" size={30} iconSize={17} />
          רדאר
        </h1>
        <p className="text-xs text-[var(--faint)] mb-6">מה בוער עכשיו · ספירה ממועד הפירעון, לא מתאריך העבודה</p>

        {/* ---- Part A: two numbers, never summed — primary glass cards with
             corner orbs. Debt=rose, commitment=cyan (never the same colour,
             DESIGN.md §2). ---- */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
          <div
            className="glass-card"
            style={{
              background: "linear-gradient(135deg, rgba(251,113,133,0.20), rgba(45,18,32,0.42))",
              borderColor: "rgba(251,113,133,0.32)",
            }}
          >
            <span className="corner-glow" style={{ ["--glow-color" as string]: "rgba(251,113,133,0.26)" }} />
          ...
          <div
            className="glass-card"
            style={{
              background: "linear-gradient(135deg, rgba(56,189,248,0.18), rgba(18,28,48,0.42))",
              borderColor: "rgba(56,189,248,0.30)",
            }}
          >
            <span className="corner-glow" style={{ ["--glow-color" as string]: "rgba(56,189,248,0.25)" }} />
            <div className="glass-content">
        ...
        {/* ---- Part B: the VU meter — the signature (DESIGN.md §3b). Bars
             carry a top-lit vertical gradient of their channel tone over a
             sunken track; the critical channel glows and pulses (softly). ---- */}
        <div
          className="rounded-2xl border border-[var(--rule)] p-5 mb-8"
          style={{ background: "rgba(255,255,255,0.03)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)" }}
        >
          <div className="text-[10px] uppercase tracking-wider font-semibold text-[var(--faint)] mb-4">ערוצי גיל החוב</div>
            ...
            {radar.vu.map((c) => {
              ...
              return (
                <Link key={c.key} href={c.href} className="flex-1 flex flex-col items-center gap-2 group">
                  <div className="font-mono text-xs text-[var(--dim)] group-hover:text-[var(--ink)] transition-colors">
                    {live ? money(c.amount) : "—"}
                  </div>
                  {/* sunken track the bar rises inside — reads as a real meter */}
                  <div
                    className="w-8 sm:w-12 flex items-end justify-center rounded-lg overflow-hidden"
                    style={{ height: 112, background: "rgba(255,255,255,0.03)", border: "1px solid var(--rule)" }}
                  >
                    <div
                      className={`w-full rounded-t-md ${critical ? "pulse-crit" : ""}`}
                      style={{
                        height: h,
                        background: `linear-gradient(to top, ${c.tone}, color-mix(in srgb, ${c.tone} 55%, white))`,
                        boxShadow: live ? `0 0 14px color-mix(in srgb, ${c.tone} 40%, transparent)` : "none",
                        opacity: live ? 1 : 0.3,
                      }}
                    />
                  </div>
        ...
        <div className="space-y-5">
          {(["red", "blue", "yellow"] as Severity[]).map((sev) => {
            ...
            return (
              <section key={sev}>
                <h2 className="text-xs font-bold mb-2 flex items-center gap-1.5" style={{ color: SEV_COLOR[sev] }}>
                ...
                >
                  {items.map((a) => (
                    <Link
                      key={a.key}
                      href={a.href}
                      className="flex items-center gap-3 px-4 py-3 border-b border-[var(--rule)] last:border-b-0 hover:bg-[rgba(255,255,255,0.03)] transition-colors"
                    >
