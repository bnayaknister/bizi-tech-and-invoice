        ...
        {users.map((u) => {
          ...
          return (
            <div
              ...
              style={{
                background: u.approved ? "rgba(255,255,255,0.03)" : "rgba(252,211,77,0.06)",
            ...
            >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-1.5">
                {PERMS.map((perm) => (
                  <label
                    key={perm.key}
                    className={`flex items-center gap-2 text-xs ${isSelf ? "opacity-50" : "cursor-pointer"}`}
                  >
                    <input
                      type="checkbox"
                      checked={u[perm.key]}
              ...
              {isSelf && (
                <div className="mt-2 text-[10px] text-[var(--faint)]">אי אפשר לשנות את ההרשאות של עצמך.</div>
              )}
            </div>
          );
        })}
