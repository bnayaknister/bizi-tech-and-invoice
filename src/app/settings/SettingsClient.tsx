...
export default function SettingsClient({
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      <h1 className="text-lg font-bold mb-4">הגדרות</h1>

      {error && (
        <div className="mb-3 text-xs text-[var(--peak)] border border-[var(--peak)] rounded-lg px-3 py-2">{error}</div>
      )}

      <section className="border border-[var(--rule)] rounded-lg bg-[var(--panel2)] p-4">
        <div className="flex items-center justify-between gap-4">
          ...
          <button
            ...
            style={{ background: enabled ? "var(--violet)" : "var(--panel3)" }}
          >
            <span
        ...
        <div className="mt-4 pt-4 border-t border-[var(--rule)] flex items-center justify-between gap-4">
          ...
          <button
            onClick={syncNow}
            disabled={syncing}
            className="text-xs border border-[var(--rule)] rounded-lg px-3 py-1.5 text-[var(--dim)] hover:bg-[var(--panel3)] disabled:opacity-50 shrink-0"
          >
        ...
        {syncResult && (
          <div className="mt-3 text-xs text-[var(--dim)] border border-[var(--rule)] rounded-lg px-3 py-2 font-mono">
            {syncResult}
          </div>
        )}
      </section>
      ...
      {confirming && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50" onClick={() => setConfirming(false)}>
          <div onClick={(e) => e.stopPropagation()} className="w-full max-w-sm border border-[var(--rule2)] rounded-xl bg-[var(--panel2)] p-5">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="border border-[var(--rule)] rounded-lg px-4 py-2 text-sm text-[var(--dim)]"
              >
                ביטול
              </button>
            </div>
